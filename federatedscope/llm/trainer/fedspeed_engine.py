...
class FedSpeedEngine:
    ...
    def __init__(self, model, config, device, process_group, initial_shards, ref_counts):
        ...
        # 状态
        self.aggregation_mode = config.federate.get('aggregation_mode', 'gradient') # 默认为梯度聚合
    ...
    def _load_shards_and_build_maps(self, initial_shards, id_to_param_map_keys):
        ...
        # 我们用服务端发来的ID列表来重建id_to_param_map
    ...
    def _initialize_optimizer(self):
        ...
        elif self.aggregation_mode == 'model':
    ...
    def _pre_forward_hook_fn(self, module, inputs):
        """在前向传播进入一个子模块之前，重建该模块所需的所有参数。"""
        logger.debug(f"[PRE-FWD HOOK @ Rank {self.rank}] For {type(module).__name__}")

        params_to_gather = []
        for param in module._parameters.values():
            if param is not None and hasattr(param, 'fedspeed_status') and param.fedspeed_status == "SHARDED":
                params_to_gather.append(param)
        if not params_to_gather:
            return
        with torch.no_grad():
            for param in params_to_gather:
    ...
    def _post_backward_hook_fn(self, module, grad_input, grad_output):
        ...
        param_ids_in_module = self.module_to_params_map.get(module.fedspeed_id, [])
        with torch.no_grad():
            for param_id in param_ids_in_module:
                if param_id in self.param_bwd_ref_count:
                    self.param_bwd_ref_count[param_id] -= 1
                    if self.param_bwd_ref_count[param_id] == 0:
                        param = self.id_to_param_map[param_id]
                        if param.fedspeed_status == "FULL":
                            param.data = torch.empty(0, dtype=param.dtype, device=param.device)
                            param.fedspeed_status = "SHARDED"
                            del self.param_bwd_ref_count[param_id]
    ...
    def aggregate_gradients(self):
        """
        【最终修正版】使用 all_reduce + 本地切片的方式，兼容 gloo 后端。
        """
    ...
    def _model_aggregation_step(self):
        ...
        if self.current_step % self.aggregation_steps == 0:
            logger.info(f"--- [FedSpeed-Model] Aggregating trainable params at Global Step #{self.current_step} ---")
    ...
    def get_my_final_shard(self):
        """
        用于在训练结束后，获取最终的、训练好的FP32主参数分片。
        同时，为了能正确地重组，我们需要知道所有可训练参数的名称和顺序。
        """
        if self.fp32_master_param_shard is not None:
            # 保证顺序对于重组至关重要
            ordered_trainable_names = sorted(list(self.trainable_param_names))
            return {
                'fp32_master_shard': self.fp32_master_param_shard.detach().cpu(),
                'ordered_trainable_names': ordered_trainable_names
            }
        return {}
    ...
    def profile_and_shard(self):
        ...
        with torch.no_grad():
            for name, param in self.model.named_parameters():
                if param.requires_grad and aggregation_mode == 'model':
                    # **不分片**，每个客户端都获得一个完整的副本
                    for rank in range(self.world_size):
                        all_param_shards[rank][name] = {
                            # 'shard_data' 这个键名虽然不准确了，但为了保持一致性，我们继续用它
                            # 它现在存储的是【完整】数据
